...
//  std::cout<<"The H_qpoases";
//   if (real_allocated)
//   {
//     std::cout<<"The H_qpoases"<< H_qpoases[0];
//     std::cout<<"The A_qpoases"<< A_qpoases[0];
...
void solve_mpc(update_data_t *update, problem_setup *setup)
{
  ...
  ct_ss_mats(I_world, rs.m, rs.r_feet, Rb, A_ct, B_ct_r);

  // Rotation of Foot:
  Matrix<fpt, 3, 3> R_foot_L;
  Matrix<fpt, 3, 3> R_foot_R;

  R_foot_L=update->L_foot;
  R_foot_R=update->R_foot;
  ...
  for (s16 leg = 0; leg < 2; leg ++){
    ...
    for(s16 i = 0; i < setup->horizon; i++){
      for (s16 j = 0; j < 4; j++){  // bound
        ...
        //std::cout<<"腿"<<leg<<"的gait:"<<(unsigned int)update->gait[2*i + leg]<<'\n';
  ...
  Matrix<fpt,1,3> lt_vec,wt_vec;
  Matrix<fpt,1,3> lt_3D;
  lt_vec << 0, 0, lt;
  wt_vec<<0,0,wt;

  Matrix<fpt,1,3> lh_vec,wh_vec;
  Matrix<fpt,1,3> lh_3D;
  lh_vec << 0, 0, lh;
  wh_vec<<0,0,wh;

  Matrix<fpt,1,3> M_vec,M_vec_x;
  M_vec << 0, 1.0, 0;
  M_vec_x<<1,0,0;
...
//leg 1
  ...
  F_control.block<1, 12>(4, 0) //Line Leg 1
      << -lt_vec * R_foot_R.transpose()* rs.R.transpose(),   0, 0, 0,  M_vec * R_foot_R.transpose()* rs.R.transpose(),  0, 0, 0;
  F_control.block<1, 12>(5, 0)
      <<  -lh_vec * R_foot_R.transpose()* rs.R.transpose(),   0, 0, 0, -M_vec * R_foot_R.transpose()* rs.R.transpose(),  0, 0, 0;
  F_control.block<1, 12>(6, 0) //Line Leg 1
      << -wt_vec * R_foot_R.transpose()* rs.R.transpose(),   0, 0, 0,  M_vec_x * R_foot_R.transpose()* rs.R.transpose(),  0, 0, 0;
  F_control.block<1, 12>(7, 0)
      <<  -wh_vec * R_foot_R.transpose()* rs.R.transpose(),   0, 0, 0, -M_vec_x * R_foot_R.transpose()* rs.R.transpose(),  0, 0, 0;
...
//leg 2
  ...
  F_control.block<1, 12>(13, 0) //Line Leg 2
      << 0, 0, 0,     -lt_vec * R_foot_L.transpose()* rs.R.transpose(), 0, 0, 0,    M_vec * R_foot_L.transpose()* rs.R.transpose();
  F_control.block<1, 12>(14, 0)
      << 0, 0, 0,    -lh_vec * R_foot_L.transpose()* rs.R.transpose(),  0, 0, 0,   -M_vec * R_foot_L.transpose()* rs.R.transpose();
  F_control.block<1, 12>(15, 0) //Line Leg 2
      << 0, 0, 0,    -wt_vec * R_foot_L.transpose()* rs.R.transpose(), 0, 0, 0,    M_vec_x * R_foot_L.transpose()* rs.R.transpose();
  F_control.block<1, 12>(16, 0)
      << 0, 0, 0,    -wh_vec * R_foot_L.transpose()* rs.R.transpose(),  0, 0, 0,  -M_vec_x * R_foot_L.transpose()* rs.R.transpose();
