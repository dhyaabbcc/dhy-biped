...
void KeyBoard::changeValue(){
    ...
    // if(userValue.ifAUTO==true)
    // {
    //     userValue.ly=min<float>(userValue.ly+sensitivityLeft, 1.25);
    //     return;
    // }
    switch (_c){
    case 'w':case 'W':
        userValue.ly = min<float>(userValue.ly+sensitivityLeft, 1.5);
        break;
    case 's':case 'S':
...
void* KeyBoard::run(void *arg){
    while(1){
        ...
        // if(userValue.ifAUTO==true)
        // {
        //       userValue.ly = min<float>(userValue.ly+sensitivityLeft, 1.2);
        // }
        usleep(1000);
    }
}