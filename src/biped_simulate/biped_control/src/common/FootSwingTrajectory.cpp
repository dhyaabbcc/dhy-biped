...
void FootSwingTrajectory<T>::computeSwingTrajectoryBezier(T phase, T swingTime) {
  ...
  _a=Interpolate::cubicBezierSecondDerivative<Vec3<T>>(_p0, _pf, phase);
  ...
  if(phase < T(0.5)) {
    ...
    za = Interpolate::cubicBezierSecondDerivative<T>(_p0[2], _p0[2] + _height, phase * 2);
    ...
    za = Interpolate::cubicBezierSecondDerivative<T>(_p0[2] + _height, _pf[2], phase * 2 - 1);
